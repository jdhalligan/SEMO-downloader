...
getfilenames:{[sd;ed;url;b]
  t:system"bash code/linux/curlscript.sh ",.semo.filesearchlink[sd;ed;url;b];
  a:"\"" vs raze read0`:./code/linux/curl.out;
  a 2+where a like "*ResourceName*"}

getfile:{[filename]                     / returns contents of input file
  system"curl https://reports.semopx.com/documents/" ,filename}
...
imbalancepricereport:{files:.semo.getfiles[x;x+1;.semo.iprurl;0];
  ...
  $[count files;`starttime xasc (uj/).semo.imbalancepricereporttable each files;]}
...
fourdayaggrollwindunitfcst:{files:.semo.getfiles[x;x+1;.semo.fdarufurl;0];
  $[count files;`starttime xasc (uj/) .semo.createfourdayaggrollwindunitfcst each files;]}
...
linearorders:{files:.semo.getfiles[x;x+1;.semo.mrurl;0];
  $[count files;`datetime xasc (uj/) tbl where 0<(count each tbl:.semo.createlo each files);]}

/ creates the complex order table from date x
complexorders:{files:.semo.getfiles[x;x+1;.semo.mrurl;0];
  $[count files;`datetime xasc (uj/) tbl where 0<(count each tbl:.semo.createco each files);]}

/ creates the index prices table from date x
indexprices:{files:.semo.getfiles[x;x+1;.semo.mrurl;0];
  $[count files;`datetime xasc (uj/) tbl where 0<(count each tbl:.semo.createipx each files);]}
...
annualloadforecast:{text:.semo.getfiles[x;x+1;.semo.aurl;0];
...
dailyloadforecast:{text:.semo.getfiles[x;x+1;.semo.dlfurl;0];
  text:text c:where 10<count each text;
...
.timer.repeat[09:00+.z.d;0W;1D00:00:00;(`.semo.savedown;`);"Save SEMO market data"]
